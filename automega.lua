...
local MAX_HOP_ATTEMPTS = 10 -- Maximum server hops before giving up
...
local function fetchAndCacheNewServers()
    ...
    for _, server in ipairs(serverData.data) do
        if server.id ~= CurrentJobId and server.playing < server.maxPlayers then
            table.insert(availableServers, server.id) -- Only store the Job ID
        end
    end

    if #availableServers > 0 then
        print("[Server Hop] Found " .. #availableServers .. " available servers. Caching their IDs.")
        local jsonString = HttpService:JSONEncode(availableServers)
        writefile(CACHE_FILE, jsonString)
        print("[Server Hop] Server IDs successfully cached to " .. CACHE_FILE)
        return true
    else
        print("[Server Hop] No other available servers found to create a new cache.")
        return false
    end
end
...
local function getAndUseCachedServer()
    ...
    local success, serverList = pcall(function() return HttpService:JSONDecode(jsonString) end)
    if not success or type(serverList) ~= "table" or #serverList == 0 then
        warn("[Server Hop] Cache file is corrupted or empty.")
        return false
    end
    ...
    print("[Server Hop] Hopping to server: " .. targetJobId)
    print("[Server Hop] " .. #serverList .. " server(s) remaining in cache.")

    -- Update the cache file with the remaining servers
    local newJsonString = HttpService:JSONEncode(serverList)
    ...
    if not tpSuccess then
        ...
        -- If teleport fails, we might want to try another server from the cache next time
    end
...
local function hopToNewServer()
    ...
    if not usedCache then
        print("[Server Hop] Cache is empty or invalid. Fetching a new list of servers from the API.")
        local fetchedNew = fetchAndCacheNewServers()

        if fetchedNew then
            print("[Server Hop] New list cached. Retrying hop using the new cache.")
            -- Retry hopping immediately after creating the cache
            getAndUseCachedServer()
        else
            print("[Server Hop] Failed to create a new server cache. Cannot hop.")
            isHopping = false
        end
    end
end
...
if not game:IsLoaded() then
    print("[Megalodon] ⏳ Waiting for game to load completely...")
    game.Loaded:Wait()
end

-- Additional wait for workspace to stabilize
task.wait(3)

-- Perform initial megalodon check (only if we want to enable auto megalodon hunt)
-- This can be controlled by enabling auto megalodon at script start
local shouldCheckMegalodon = true -- Set this to true to enable initial megalodon checking
...
if shouldCheckMegalodon then
    ...
    if not canContinue then
        ...
        return -- Exit current script execution
    end
end

print("🚀 [Auto Fish] All systems initialized successfully!")
print("📋 [Auto Fish] Server hop system: " .. (hasFileSupport and "Enabled" or "Disabled (no file support)"))
print("🔧 [Auto Fish] Loading main script components...")
