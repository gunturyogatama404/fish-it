...
-- ====================================================================
--                  RENDERING OPTIMIZATION - DIRECT3D 10
-- ====================================================================
-- Force Direct3D 10 for better performance on older hardware
-- This reduces CPU/GPU overhead compared to D3D11
--
-- Note: This will slightly reduce visual quality but significantly
-- improve performance, especially on older/potato PCs
-- ====================================================================

print("[Performance] Applying Direct3D 10 rendering optimizations...")

local function applyRenderingFFlags()
    local fflagsApplied = 0
    local fflagsFailed = 0

    -- Check if setfflag function exists
    if not setfflag then
        warn("⚠️ [Rendering] setfflag() not available in this executor")
        return false
    end

    local renderingFlags = {
        -- Force Direct3D 10 mode
        {name = "FFlagDebugGraphicsDisableDirect3D11", value = "false"},
        {name = "FFlagDebugGraphicsPreferD3D11", value = "false"},
        {name = "FFlagDebugGraphicsPreferD3D11FL10", value = "true"},
        {name = "FFlagGraphicsEnableD3D10Compute", value = "true"},

        -- Additional performance optimizations
        {name = "FFlagDebugGraphicsDisableVulkan", value = "true"},  -- Disable Vulkan fallback
        {name = "FFlagGraphicsD3D10CompatMode", value = "true"},     -- Enable D3D10 compat mode
    }

    for _, flag in ipairs(renderingFlags) do
        local success = pcall(function()
            setfflag(flag.name, flag.value)
        end)

        if success then
            fflagsApplied = fflagsApplied + 1
            print(string.format("[Rendering] ✅ Applied: %s = %s", flag.name, flag.value))
        else
            fflagsFailed = fflagsFailed + 1
            warn(string.format("[Rendering] ❌ Failed: %s", flag.name))
        end
    end

    print(string.format("[Rendering] FFlags applied: %d/%d (Failed: %d)",
        fflagsApplied, #renderingFlags, fflagsFailed))

    return fflagsApplied > 0
end

-- Apply rendering flags
local renderingSuccess = pcall(applyRenderingFFlags)
if not renderingSuccess then
    warn("⚠️ [Rendering] Failed to apply Direct3D 10 flags, continuing with default settings...")
else
    print("[Rendering] ✅ Direct3D 10 mode configured successfully!")
end
...
function enableGPUSaver()
    ...
    -- ====== AUTOMATIC PERFORMANCE OPTIMIZATION (PHASE 2: GPU SAVER) ======
