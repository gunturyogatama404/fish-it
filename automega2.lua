...
local CACHE_FILE = "megalodon_server_cache.json" -- File to store server IDs
local MEGALODON_CHECK_DELAY = 10 -- seconds to wait before checking for megalodon
...
print("=== MEGALODON SERVER HOP SYSTEM ===")
print("Target Place ID:", PlaceID)
print("Current Server ID:", CurrentJobId)
print("File Support:", hasFileSupport and "Available" or "Not Available")

-- Function to fetch a new list of servers from the API and cache them
local function fetchAndCacheNewServers()
    if not hasFileSupport then
        warn("[Server Hop] File system not supported by this executor")
        return false
    end

    print("[Server Hop] Searching for new servers to cache...")
    local serverDataResponse = nil
    ...
    for attempt = 1, MAX_RETRIES do
        ...
        if success then
            ...
            warn("[Server Hop] Attempt " .. attempt .. ": Failed to get server list. Details:", response)
            if attempt < MAX_RETRIES then
                print("[Server Hop] Retrying in " .. RETRY_DELAY .. " seconds...")
                task.wait(RETRY_DELAY)
            else
                print("[Server Hop] All " .. MAX_RETRIES .. " attempts failed. Cannot fetch server list.")
                return false
            end
        end
    end

    if not serverDataResponse then return false end

    local success, serverData = pcall(function()
        return HttpService:JSONDecode(serverDataResponse)
    end)

    if not success or not serverData or not serverData.data then
        warn("[Server Hop] Failed to decode server list JSON or data is invalid.")
        return false
    end

    -- Load existing used servers to avoid them
    local usedServers = {}
    if isfile(CACHE_FILE) then
        local existingSuccess, existingContent = pcall(function()
            return readfile(CACHE_FILE)
        end)
        if existingSuccess and existingContent ~= "" then
            local existingSuccess2, existingData = pcall(function()
                return HttpService:JSONDecode(existingContent)
            end)
            if existingSuccess2 and existingData.usedServers then
                usedServers = existingData.usedServers
            end
        end
    end
    ...
    if #availableServers > 0 then
        ...
        print("[Server Hop] No unused available servers found. Clearing used servers list and retrying...")
        -- Clear used servers if no fresh servers available
        local cacheData = {
            servers = {},
            lastUpdated = os.time(),
            usedServers = {},
...
local function shouldRefreshCache(cacheData)
    if not cacheData or not cacheData.lastUpdated then
        return true, "No timestamp found"
    end

    local cacheAge = os.time() - cacheData.lastUpdated
    if cacheAge > MAX_CACHE_AGE then
        return true, string.format("Cache too old (%d minutes)", math.floor(cacheAge/60))
    end

    if cacheAge > SERVER_REFRESH_INTERVAL then
        return true, string.format("Scheduled refresh (%d minutes)", math.floor(cacheAge/60))
    end

    return false, string.format("Cache fresh (%d minutes old)", math.floor(cacheAge/60))
end

-- Function to get a server from the cache and teleport
local function getAndUseCachedServer()
    if not hasFileSupport then return false end

    if not isfile(CACHE_FILE) then
        print("[Server Hop] Cache file not found.")
        return false -- Indicates that a new list needs to be fetched
    end

    local success, jsonString = pcall(function() return readfile(CACHE_FILE) end)
    if not success or jsonString == "" then
        warn("[Server Hop] Could not read cache file or it is empty.")
        return false
    end

    local success, cacheData = pcall(function() return HttpService:JSONDecode(jsonString) end)
    if not success or type(cacheData) ~= "table" then
        warn("[Server Hop] Cache file is corrupted.")
        return false
    end

    -- Handle both old and new cache formats
    local serverList, usedServers
    if cacheData.servers then
        -- New enhanced format
        serverList = cacheData.servers or {}
        usedServers = cacheData.usedServers or {}

        -- Check if cache needs refresh
        local needsRefresh, reason = shouldRefreshCache(cacheData)
        if needsRefresh then
            print("[Server Hop] " .. reason .. ". Triggering cache refresh...")
            return false
        end
    else
        -- Old format (array of server IDs)
        serverList = cacheData
        usedServers = {}
        print("[Server Hop] Using legacy cache format, will upgrade on next refresh.")
    end

    if #serverList == 0 then
        print("[Server Hop] No servers available in cache.")
        return false
    end
    ...
    if not tpSuccess then
        ...
        print("[Server Hop] Removed failed server from used list, will retry with next server.")

        -- Update cache without the bad server
        updatedCacheData.usedServers = usedServers
...
local function hopToNewServer()
    ...
    while retryCount < maxRetries and not successfulHop do
        ...
        if usedCache then
            ...
            print("[Server Hop] Cache attempt " .. retryCount .. " failed.")

            if retryCount < maxRetries then
                print("[Server Hop] Fetching fresh server list...")
                local fetchedNew = fetchAndCacheNewServers()

                if not fetchedNew then
                    print("[Server Hop] Failed to fetch new servers, waiting before retry...")
                    task.wait(2) -- Brief wait before retry
                end
            end
        end
    end
...
local function sendMegalodonEventWebhook(statusType)
    ...
    pcall(function()
        local req = (syn and syn.request) or http_request
        if req then
            req({ Url=webhook2, Method="POST", Headers={["Content-Type"]="application/json"}, Body=HttpService:JSONEncode(payload) })
        end
    end)
end

local function setupMegalodonMonitor(propsFolderToWatch)
    if not propsFolderToWatch or not propsFolderToWatch:IsA("Instance") then
        warn("[Megalodon Monitor] Invalid instance provided. Monitor not started.")
        return
    end

    print("[Megalodon Monitor] ✅ Attaching efficient event listener to: " .. propsFolderToWatch:GetFullName())

    local connection
    connection = propsFolderToWatch.AncestryChanged:Connect(function(child, parent)
        if parent == nil then
            print("[Megalodon Monitor] ❗ Event has ended (Props folder removed).")

            if connection then
                connection:Disconnect()
            end

            sendMegalodonEventWebhook("hopping")
            print("[Megalodon Monitor] 🔄 Initiating server hop...")
            task.wait(2) -- Brief delay for webhook
            hopToNewServer()
        end
    end)
end

-- Function to perform initial megalodon check
local function performInitialMegalodonCheck()
    print("[Megalodon] 🔍 Performing initial server check for Megalodon event...")

    -- Wait for workspace to be fully loaded
    if not workspace:FindFirstChild("!!! MENU RINGS") then
        print("[Megalodon] ⏳ Waiting for workspace to fully load...")
        local waitTime = 0
        repeat
            task.wait(1)
            waitTime = waitTime + 1
        until workspace:FindFirstChild("!!! MENU RINGS") or waitTime > 15
    end

    -- Check for megalodon
    local eventFound = false
    local targetPropsFolder = nil
        ...
        if menuRings then
            for _, propsFolder in ipairs(menuRings:GetChildren()) do
                if propsFolder.Name == "Props" then
                    ...
                    if huntFolder and huntFolder:FindFirstChild("Color") then
                        eventFound = true
                        targetPropsFolder = propsFolder
                        break
                    end
                end
            end
    ...
    if eventFound then
        ...
        -- Only hop if we haven't reached the max attempts yet
        if hopAttempts < MAX_HOP_ATTEMPTS then
            print("[Megalodon] 🔄 Attempting to find a server with Megalodon event...")
            hopToNewServer()
            return false -- Script will be terminated by server hop
        else
            print("[Megalodon] ⚠️ Maximum hop attempts reached. Continuing without Megalodon hunting...")
            return true
        end
    end
end

-- Wait for game to be fully loaded before performing checks
if not game:IsLoaded() then
    print("[Auto Fish] ⏳ Waiting for game to load completely...")
    game.Loaded:Wait()
end

print("🚀 [Auto Fish] Loading main script components first...")
print("📋 [Auto Fish] Server hop system: " .. (hasFileSupport and "Enabled" or "Disabled (no file support)"))

-- Load main script components FIRST
task.wait(1)
LightweightInventory.start(DiscordNotifier.scanInventory)

print("✅ [Auto Fish] Main script components loaded successfully!")

-- Now schedule megalodon check with delay (5-8 seconds as requested)
local shouldCheckMegalodon = true -- Set this to false to disable initial megalodon checking
local megalodonCheckDelay = math.random(5, 8) -- Random delay between 5-8 seconds

if shouldCheckMegalodon then
    print("[Megalodon] 📅 Scheduling megalodon check in " .. megalodonCheckDelay .. " seconds after main script load...")
    ...
    print("[Megalodon] ⚠️ Megalodon auto-hunt disabled. Script will run without server hopping.")
end
...
local function autoDetectMegalodon()
    ...
    if eventFound and eventPosition then
        ...
        if not megalodonEventActive then
            ...
            print("[Megalodon] ✅ Event found! Script will continue normally.")
        end
        ...
        elseif not megalodonMissingAlertSent then
            ...
            if isAutoMegalodonOn and not isHopping then
                ...
                task.spawn(function()
                    task.wait(2) -- Wait 2 seconds before hopping
                    hopToNewServer()
                end)
            end
        end
